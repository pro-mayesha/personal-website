...
import { BLOG_CATEGORIES, getPostExcerpt, normalizePostCategory } from "./blogCategories";
    ...
    >
      ...
      <div className="relative z-[1] flex flex-1 flex-col p-5 md:p-6">
        ...
        <h3 className="mb-2 font-hand text-[22px] font-bold leading-tight text-ink transition-colors group-hover:text-terracotta md:text-[26px]">
          <Link href={`/notes/${post.slug}`} className="outline-none focus-visible:underline">
            {post.title}
          </Link>
        </h3>
        ...
        <p className="mb-4 flex-1 font-garamond text-[15px] leading-relaxed text-ink/55 line-clamp-3">{excerpt}</p>

        <span className="font-hand text-[17px] text-terracotta/70 transition-colors group-hover:text-terracotta">
          read note →
        </span>
      </div>
    </article>
